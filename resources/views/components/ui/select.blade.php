...
<select
    ...
    {{ $attributes->class('flex w-full rounded-[var(--radius-button)] bg-[var(--color-panel)] text-[var(--color-ink)] shadow-sm outline-none transition-colors disabled:cursor-not-allowed disabled:border-[var(--color-line)] disabled:bg-[color-mix(in_srgb,var(--color-panel)_88%,var(--color-page))] disabled:text-[color-mix(in_srgb,var(--color-muted)_92%,white)] '.$sizes[$size].' '.$classes) }}
>
    {{ $slot }}
</select>
