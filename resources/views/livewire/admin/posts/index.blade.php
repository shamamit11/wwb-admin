    ...
    <x-ui.table caption="Posts" density="compact">
        <x-ui.table-head>
            <tr>
                ...
                @if ($aiReviewMode)
                    <x-ui.table-heading class="w-[14%]">SOURCE BRIEF</x-ui.table-heading>
                    ...
                    @if ($aiReviewMode)
                        <x-ui.table-cell subdued class="align-top">
                            <div class="space-y-1.5">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--color-muted)]">Brief</p>
                                @if ($post['source_content_brief_id'])
                                    <a href="{{ route('content-briefs.show', ['contentBrief' => $post['source_content_brief_id']]) }}" class="inline-flex rounded-full bg-[var(--color-panel-soft)] px-3 py-1.5 text-sm font-medium text-[var(--color-ink)] transition-colors hover:bg-[color-mix(in_srgb,var(--color-panel-soft)_70%,white)]">
