@php
    $summaryItems = [
        [
            'label' => 'Hero',
            'detail' => $hero['title'] ?: 'No hero title yet',
            'tone' => $hero['title'] ? 'default' : 'warning',
        ],
        [
            'label' => 'Featured Editorial',
            'detail' => ($featured_editorial['limit'] ?? 3).' item limit',
            'tone' => 'default',
        ],
        [
            'label' => 'Recent Articles',
            'detail' => ($guide_section['limit'] ?? 4).' item limit',
            'tone' => 'default',
        ],
        [
            'label' => 'Explore Core Topics',
            'detail' => 'Automatic from active categories',
            'tone' => 'default',
        ],
        [
            'label' => 'Promo Block',
            'detail' => (($promo_section['enabled'] ?? false) ? 'Enabled' : 'Disabled').' · '.count($promo_section['bullet_points'] ?? []).' bullets',
            'tone' => ($promo_section['enabled'] ?? false) ? 'default' : 'muted',
        ],
        [
            'label' => 'Newsletter',
            'detail' => ($newsletter_section['enabled'] ?? false) ? 'Enabled' : 'Disabled',
            'tone' => ($newsletter_section['enabled'] ?? false) ? 'default' : 'muted',
        ],
        [
            'label' => 'Homepage SEO',
            'detail' => $seo['meta_title'] ?: 'No meta title yet',
            'tone' => $seo['meta_title'] ? 'default' : 'warning',
        ],
    ];

    $summaryCardClasses = [
        'default' => 'border-[var(--color-line)] bg-[var(--color-panel-soft)]',
        'muted' => 'border-[var(--color-line)] bg-[color-mix(in_srgb,var(--color-page)_75%,white)]',
        'warning' => 'border-[color-mix(in_srgb,var(--color-warning)_24%,white)] bg-[color-mix(in_srgb,var(--color-warning)_8%,white)]',
    ];

    $sections = [
        ['id' => 'homepage-hero', 'label' => 'Hero', 'detail' => $hero['title'] ?: 'Needs title'],
        ['id' => 'homepage-featured', 'label' => 'Featured Editorial', 'detail' => 'Auto-populated'],
        ['id' => 'homepage-guides', 'label' => 'Recent Articles', 'detail' => 'Auto-populated'],
        ['id' => 'homepage-topics', 'label' => 'Explore Core Topics', 'detail' => 'Auto-populated'],
        ['id' => 'homepage-promo', 'label' => 'Promo Block', 'detail' => ($promo_section['enabled'] ?? false) ? 'Enabled' : 'Disabled'],
        ['id' => 'homepage-newsletter', 'label' => 'Newsletter', 'detail' => ($newsletter_section['enabled'] ?? false) ? 'Enabled' : 'Disabled'],
        ['id' => 'homepage-seo', 'label' => 'Homepage SEO', 'detail' => $seo['meta_title'] ? 'Configured' : 'Meta title missing'],
    ];
@endphp
...
<div class="space-y-6">
    ...
    >
        ...
        <x-ui.button as="a" :href="config('app.url')" variant="secondary" target="_blank" rel="noreferrer">Preview Homepage</x-ui.button>
    ...
    <div class="grid gap-6 xl:grid-cols-[minmax(0,1.42fr)_21rem]">
        <div class="space-y-4">
            <section id="homepage-hero" x-data="{ open: true }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        ...
                        <p class="text-sm text-[var(--color-muted)]">Lead with the homepage’s main promise, core message, CTAs, and media reference.</p>
                    ...
                    <button
                        ...
                        class="inline-flex items-center justify-center gap-2 rounded-[var(--radius-button)] border border-[var(--color-line)] bg-[var(--color-panel-soft)] px-3 py-2 text-sm font-medium text-[var(--color-ink)] transition-colors hover:bg-[var(--color-page)]"
                    >
                        <span x-text="open ? 'Collapse' : 'Expand'"></span>
                        <svg class="h-4 w-4 transition-transform" x-bind:class="open ? 'rotate-180' : ''" viewBox="0 0 20 20" fill="none" aria-hidden="true">
                ...
                <div x-cloak x-show="open" x-transition.opacity.duration.150ms class="space-y-5 border-t border-[var(--color-line)] px-6 py-6">
                    ...
                    <div class="grid gap-5 lg:grid-cols-2">
                        <x-ui.field label="Media URL" for="homepage-hero-media-url" :error="$errors->first('hero.media_url')" hint="Use a full CDN or media-library URL so editors can verify the exact homepage asset.">
            ...
            <section id="homepage-featured" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-[var(--color-ink)]">Featured Editorial</h2>
                            <x-ui.badge tone="success">Auto-populated</x-ui.badge>
                            <x-ui.badge tone="muted">Limit: {{ $featured_editorial['limit'] }}</x-ui.badge>
                        </div>
                        <p class="text-sm text-[var(--color-muted)]">This section is populated from featured published posts.</p>
                        <p class="text-sm text-[var(--color-muted)]">Editors control the label, description, and item limit only.</p>
                    </div>
                ...
                <div x-cloak x-show="open" x-transition.opacity.duration.150ms class="grid gap-5 border-t border-[var(--color-line)] px-6 py-6 lg:grid-cols-[minmax(0,1fr)_11rem]">
                    <div class="space-y-5">
            ...
            <section id="homepage-guides" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-[var(--color-ink)]">Recent Articles</h2>
                            <x-ui.badge tone="success">Auto-populated</x-ui.badge>
                            <x-ui.badge tone="muted">Limit: {{ $guide_section['limit'] }}</x-ui.badge>
                        </div>
                        <p class="text-sm text-[var(--color-muted)]">This section is populated from recent published posts.</p>
                        <p class="text-sm text-[var(--color-muted)]">Editors control the label, description, and item limit only.</p>
                    </div>
            ...
            <section id="homepage-topics" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                        ...
                        <p class="text-sm text-[var(--color-muted)]">This section is populated from all active categories.</p>
                        <p class="text-sm text-[var(--color-muted)]">Editors control the label and supporting copy only.</p>
                    </div>
            ...
            <section id="homepage-promo" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-[var(--color-ink)]">Promo Section</h2>
                            <x-ui.badge :tone="$promo_section['enabled'] ? 'success' : 'muted'">{{ $promo_section['enabled'] ? 'Enabled' : 'Disabled' }}</x-ui.badge>
                        </div>
                        <p class="text-sm text-[var(--color-muted)]">Configure the homepage promotional resource block.</p>
                        <p class="text-sm text-[var(--color-muted)]">{{ count($promo_section['bullet_points'] ?? []) }} bullets · {{ count($promo_section['stats'] ?? []) }} stats</p>
                    </div>

                    <div class="flex items-center gap-3">
                        <label x-on:click.stop class="flex items-center gap-3 rounded-[var(--radius-button)] border border-[var(--color-line)] bg-[var(--color-panel-soft)] px-4 py-3">
                            ...
                            <span class="text-sm font-medium text-[var(--color-ink)]">{{ $promo_section['enabled'] ? 'Enabled' : 'Disabled' }}</span>
                        </label>
                ...
                <div x-cloak x-show="open" x-transition.opacity.duration.150ms class="border-t border-[var(--color-line)] px-6 py-6">
                    <div class="space-y-5 {{ $promo_section['enabled'] ? '' : 'opacity-60' }}">
                        <div class="grid gap-5 lg:grid-cols-2">
            ...
            <section id="homepage-newsletter" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-[var(--color-ink)]">Newsletter Section</h2>
                            <x-ui.badge :tone="$newsletter_section['enabled'] ? 'success' : 'muted'">{{ $newsletter_section['enabled'] ? 'Enabled' : 'Disabled' }}</x-ui.badge>
                        </div>
                        <p class="text-sm text-[var(--color-muted)]">Manage the newsletter CTA copy without mixing it into generic settings.</p>
                        <p class="text-sm text-[var(--color-muted)]">{{ $newsletter_section['title'] ?: 'No newsletter title yet' }}</p>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <label x-on:click.stop class="flex items-center gap-3 rounded-[var(--radius-button)] border border-[var(--color-line)] bg-[var(--color-panel-soft)] px-4 py-3">
                            ...
                            <span class="text-sm font-medium text-[var(--color-ink)]">{{ $newsletter_section['enabled'] ? 'Enabled' : 'Disabled' }}</span>
                        </label>
                ...
                <div x-cloak x-show="open" x-transition.opacity.duration.150ms class="border-t border-[var(--color-line)] px-6 py-6">
                    <div class="space-y-5 {{ $newsletter_section['enabled'] ? '' : 'opacity-60' }}">
            ...
            <section id="homepage-seo" x-data="{ open: false }" class="rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] shadow-[var(--shadow-card)]">
                <div class="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="space-y-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <h2 class="text-lg font-semibold tracking-[-0.02em] text-[var(--color-ink)]">Homepage SEO</h2>
                            <x-ui.badge :tone="$seo['meta_title'] ? 'success' : 'warning'">{{ $seo['meta_title'] ? 'Configured' : 'SEO Missing' }}</x-ui.badge>
                        </div>
                        <p class="text-sm text-[var(--color-muted)]">Keep homepage-specific search metadata separate from page builder content.</p>
        ...
        <aside class="space-y-6 xl:sticky xl:top-[5.5rem] xl:self-start">
            <section class="space-y-5 rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-panel)] px-5 py-5 shadow-[var(--shadow-card)]">
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-[var(--color-muted)]">A quick overview of section readiness, automatic sources, and disabled areas.</p>
                </div>
